package edu.wpi.cs3733.D21.teamB.chatbot.ab;
...
public class PCAIMLProcessorExtension implements AIMLProcessorExtension {
    ...
    private String newContact(Node node, org.alicebot.ab.ParseState ps) {
        ...
        for (int i = 0; i < childList.getLength(); i++)  {
            if (childList.item(i).getNodeName().equals("birthday")) {
                birthday = org.alicebot.ab.AIMLProcessor.evalTagContent(childList.item(i), ps, null);
            }
            if (childList.item(i).getNodeName().equals("phonetype")) {
                phoneType = org.alicebot.ab.AIMLProcessor.evalTagContent(childList.item(i), ps, null);
            }
            if (childList.item(i).getNodeName().equals("emailtype")) {
                emailType = org.alicebot.ab.AIMLProcessor.evalTagContent(childList.item(i), ps, null);
            }
            if (childList.item(i).getNodeName().equals("dialnumber")) {
                dialNumber = org.alicebot.ab.AIMLProcessor.evalTagContent(childList.item(i), ps, null);
            }
            if (childList.item(i).getNodeName().equals("displayname")) {
                displayName = org.alicebot.ab.AIMLProcessor.evalTagContent(childList.item(i), ps, null);
            }
            if (childList.item(i).getNodeName().equals("emailaddress")) {
                emailAddress = org.alicebot.ab.AIMLProcessor.evalTagContent(childList.item(i), ps, null);
            }
        }
        System.out.println("Adding new contact "+displayName+" "+phoneType+" "+dialNumber+" "+emailType+" "+emailAddress+" "+birthday);
        org.alicebot.ab.Contact contact = new org.alicebot.ab.Contact(displayName, phoneType, dialNumber, emailType, emailAddress, birthday);
        return "";
    }
    private String contactId(Node node, org.alicebot.ab.ParseState ps) {
        String displayName = org.alicebot.ab.AIMLProcessor.evalTagContent(node, ps, null);
        String result = org.alicebot.ab.Contact.contactId(displayName);
        //System.out.println("contactId("+displayName+")="+result);
        return result;
    }
    private String multipleIds(Node node, org.alicebot.ab.ParseState ps){
        String contactName = org.alicebot.ab.AIMLProcessor.evalTagContent(node, ps, null);
        String result = org.alicebot.ab.Contact.multipleIds(contactName);
        //System.out.println("multipleIds("+contactName+")="+result);
        return result;
    }
    private String displayName(Node node, org.alicebot.ab.ParseState ps){
        String id = org.alicebot.ab.AIMLProcessor.evalTagContent(node, ps, null);
        String result = org.alicebot.ab.Contact.displayName(id);
        //System.out.println("displayName("+id+")="+result);
        return result;
    }
    private String dialNumber(Node node, org.alicebot.ab.ParseState ps) {
        NodeList childList = node.getChildNodes();
        String id="unknown";
        String type="unknown";
        for (int i = 0; i < childList.getLength(); i++)  {
            if (childList.item(i).getNodeName().equals("id")) {
                id = org.alicebot.ab.AIMLProcessor.evalTagContent(childList.item(i), ps, null);
            }
            if (childList.item(i).getNodeName().equals("type")) {
                type = org.alicebot.ab.AIMLProcessor.evalTagContent(childList.item(i), ps, null);
            }
        }
        String result = org.alicebot.ab.Contact.dialNumber(type, id);
        //System.out.println("dialNumber("+id+")="+result);
        return result;
    }

    private String emailAddress(Node node, org.alicebot.ab.ParseState ps){
        NodeList childList = node.getChildNodes();
        String id="unknown";
        String type="unknown";
        for (int i = 0; i < childList.getLength(); i++)  {
        ...
        String result = org.alicebot.ab.Contact.emailAddress(type, id);
        //System.out.println("emailAddress("+id+")="+result);
        return result;
    }


    private String contactBirthday(Node node, org.alicebot.ab.ParseState ps){
        String id = org.alicebot.ab.AIMLProcessor.evalTagContent(node, ps, null);
